...
function autocomplete(input, latInput, lngInput) {
	...
	input.on('keydown', function (e) {
		if (e.keyCode == 13) e.preventsDefault();
	});
}
...
function loadPlaces(map) {
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		// create bounds
		var bounds = new google.maps.LatLngBounds(); // bounds is for displaying the markers at the very center of the map (in group)
		...
		var markers = places.map(function (place) {
			var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
			    ...
			    placeLat = _place$location$coord[1]; // array destruction				
